...
author: "Joseph Holler, Junyi Zhou, Peter Kedron, Drew An-Pham, Derrick Burt, Katie Heo"
date: "`r Sys.Date()`"
...
Version 2.1 \| First Created July 7, 2021 \| Updated December 12th, 2023
...
missing_Data <- acs_covid %>% filter( is.na(bpov_pct))%>% st_drop_geometry()
...
```{r residuals1}
# create data frame with residuals FOR RACE
...
```{r residuals2}
# create data frame with residuals FOR ETHNICITY
gee_resid2 <- data.frame(
  select(gee_or_clus_rpdep$models[[2]]$data, covid_rate, ss_clusID),
  data.frame(fitvals = gee_or_clus_rpdep$models[[2]]$fitted.values),
  data.frame(residual = gee_or_clus_rpdep$models[[2]]$residual),
  data.frame(working = residuals(gee_or_clus_rpdep$models[[2]], type = "working")),
  data.frame(response = residuals(gee_or_clus_rpdep$models[[2]], type = "response")),
  data.frame(pearson = residuals(gee_or_clus_rpdep$models[[2]], type = "pearson"))
) %>%
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_or_clus_rpdep$models[[2]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()
```

```{r residuals3}
# create data frame with residuals FOR POVERTY STATUS
gee_resid3 <- data.frame(
  select(gee_or_clus_rpdep$models[[3]]$data, covid_rate, ss_clusID),
  data.frame(fitvals = gee_or_clus_rpdep$models[[3]]$fitted.values),
  data.frame(residual = gee_or_clus_rpdep$models[[3]]$residual),
  data.frame(working = residuals(gee_or_clus_rpdep$models[[3]], type = "working")),
  data.frame(response = residuals(gee_or_clus_rpdep$models[[3]], type = "response")),
  data.frame(pearson = residuals(gee_or_clus_rpdep$models[[3]], type = "pearson"))
) %>%
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_or_clus_rpdep$models[[3]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()
```

```{r residuals4}
# create data frame with residuals FOR AGE
gee_resid4 <- data.frame(
  select(gee_or_clus_rpdep$models[[4]]$data, covid_rate, ss_clusID),
  data.frame(fitvals = gee_or_clus_rpdep$models[[4]]$fitted.values),
  data.frame(residual = gee_or_clus_rpdep$models[[4]]$residual),
  data.frame(working = residuals(gee_or_clus_rpdep$models[[4]], type = "working")),
  data.frame(response = residuals(gee_or_clus_rpdep$models[[4]], type = "response")),
  data.frame(pearson = residuals(gee_or_clus_rpdep$models[[4]], type = "pearson"))
) %>%
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_or_clus_rpdep$models[[4]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()
```

```{r residuals5}
# create data frame with residuals FOR BIOLOGICAL SEX
gee_resid5 <- data.frame(
  select(gee_or_clus_rpdep$models[[5]]$data, covid_rate, ss_clusID),
  data.frame(fitvals = gee_or_clus_rpdep$models[[5]]$fitted.values),
  data.frame(residual = gee_or_clus_rpdep$models[[5]]$residual),
  data.frame(working = residuals(gee_or_clus_rpdep$models[[5]], type = "working")),
  data.frame(response = residuals(gee_or_clus_rpdep$models[[5]], type = "response")),
  data.frame(pearson = residuals(gee_or_clus_rpdep$models[[5]], type = "pearson"))
) %>%
  mutate(
    my_working = (covid_rate - fitvals) / fitvals,
    my_pearson = response * (sqrt(gee_or_clus_rpdep$models[[5]]$prior.weights)) / sqrt(fitvals^2)
  ) %>%
  st_sf()
```
...
```{r map-residuals7}
tm_gee_residuals2 <- tm_shape(gee_resid2) +
  tm_polygons("response",
    title = "Ethnicity Model Residuals",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
```{r map-residuals8}
tm_gee_residuals3 <- tm_shape(gee_resid3) +
  tm_polygons("response",
    title = "Poverty Status Model Residuals",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
```{r map-residuals9}
tm_gee_residuals4 <- tm_shape(gee_resid4) +
  tm_polygons("response",
    title = "Age Model Residuals",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
```{r map-residuals10}
tm_gee_residuals5 <- tm_shape(gee_resid5) +
  tm_polygons("response",
    title = "Biological Sex Model Residuals",
    style = "equal",
    n = 7,
    border.alpha = .2,
    lwd = 0.2,
...
Calculate variance, covariance, correlation, and weights for race, ethnicity, poverty status, age, and biological sex.
...
```{r weights2}
# calculate k number of observations per cluster
gee_resid2 <- gee_resid2 %>% 
  group_by(ss_clusID) %>% 
  mutate(k = n()) %>% 
  ungroup()

grps <- gee_resid2 %>% st_drop_geometry() %>% select(ss_clusID, residual = residual)

# create mask matrix
n <- length(gee_resid2$ss_clusID)
# m <- matrix(nrow = n, ncol = n)
...
```{r weights3}
# calculate k number of observations per cluster
gee_resid3 <- gee_resid3 %>% 
  group_by(ss_clusID) %>% 
  mutate(k = n()) %>% 
  ungroup()

grps <- gee_resid3 %>% st_drop_geometry() %>% select(ss_clusID, residual = residual)

# create mask matrix
n <- length(gee_resid3$ss_clusID)
# m <- matrix(nrow = n, ncol = n)
...
```{r weights4}
# calculate k number of observations per cluster
gee_resid4 <- gee_resid4 %>% 
  group_by(ss_clusID) %>% 
  mutate(k = n()) %>% 
  ungroup()

grps <- gee_resid4 %>% st_drop_geometry() %>% select(ss_clusID, residual = residual)

# create mask matrix
n <- length(gee_resid4$ss_clusID)
# m <- matrix(nrow = n, ncol = n)
...
```{r weights5}
# calculate k number of observations per cluster
gee_resid5 <- gee_resid5 %>% 
  group_by(ss_clusID) %>% 
  mutate(k = n()) %>% 
  ungroup()

grps <- gee_resid5 %>% st_drop_geometry() %>% select(ss_clusID, residual = residual)
...
Map each county's weight in GEE model (race, ethnicity, poverty status, age, biological sex).
...
tm_gee_weights2 <- tm_shape(gee_resid2) +
  tm_polygons("w",
    title = "Ethnicity Model Weights",
    breaks = breaks,
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges",
...
tm_gee_weights3 <- tm_shape(gee_resid3) +
  tm_polygons("w",
    title = "Poverty Status Model Weights",
    breaks = breaks,
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges",
...
tm_gee_weights4 <- tm_shape(gee_resid4) +
  tm_polygons("w",
    title = "Age Model Weights",
    breaks = breaks,
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges",
...
tm_gee_weights5 <- tm_shape(gee_resid5) +
  tm_polygons("w",
    title = "Biological Sex Model Weights",
    breaks = breaks,
    border.alpha = .2,
    lwd = 0.2,
    palette = "Oranges",
...
We looked at each county's weight in GEE model in the five models of race, ethnicity, poverty status, age, and biological sex model weights. This led to a more accurate and comprehensive understanding of the data. The five maps did not show much variation in that states showed similar weights in the five maps.
The generation of model weights maps for race, ethnicity, poverty status, age, and biological sex serves a crucial role in elucidating the differential impact of COVID-19 on various demographic categories. By setting manual breaks at specific intervals (0, 0.2, 0.4, 0.6, 0.8, and 1.0) in the model weights maps, we facilitate a clearer comparison of regions with higher Generalized Estimating Equation (GEE) weights, indicating the counties that significantly influence the model. Setting manual breaks in this manner gives each color the same meaning. This approach allows for a more nuanced understanding of how certain regions in the east and west coast play a more pivotal role in shaping the model's outcomes. The use of residual maps complements the analysis by providing insights into the spatial distribution of errors, highlighting areas where the model deviates from the observed data. The significance lies in the absence of spatial partners or clusters in the errors, indicating the model's effectiveness in capturing the underlying patterns. 
In this study, the residual maps illustrate the misfit between the predicted values from the Generalized Estimating Equation (GEE) model and the actual COVID-19 incidence rates at the county level. Clusters or patterns in the residuals, as observed in spots in the metropolitan areas of the northeast and California, and in some rural areas, including eastern Washington, Oregon, and Arizona, suggest that certain geographic regions exhibit consistently higher or lower errors, signifying potential shortcomings in the model's representation of the spatial distribution of the outcome variable. The similarity between the residual maps and the model weights map underscores the importance of the latter in guiding the model to fit the data in counties with higher weights. This methodological transparency enhances the reliability and interpretability of the study's findings, contributing to a comprehensive evaluation of the relationship between COVID-19 and socio-demographic factors. The model weights map, derived from GEE modeling, becomes a valuable tool in identifying counties that significantly contribute to understanding the impact of COVID-19 across different demographic dimensions.

Moreover, closer inspection of the clusters for the GEE models revealed a highly skewed distribution of cluster sizes, with most clusters containing very few counties and a few clusters containing 50 or more counties.
...
These comparisons led to further insights and research into discrepancies between the original and reproduced Pearsons' correlations between various forms of the Kulldorff spatial scan statistic.
